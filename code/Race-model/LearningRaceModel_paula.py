...
class LearningRaceModel:
    ...
    def __init__(self, learning_rate, momentum, mouseNum, max_epochs, zvar, wvar, bvar, verbose):
        ...
        mouse = pd.read_csv('dopaminepl-scratch/Dataset_2020-11-03/DAP_LearningGrating2AFC_mouseInfo.csv')
        sess = pd.read_csv('dopaminepl-scratch/Dataset_2020-11-03/DAP_LearningGrating2AFC_sessionInfo.csv')
        trial = pd.read_csv('dopaminepl-scratch/Dataset_2020-11-03/DAP_LearningGrating2AFC_trialInfo.csv')
        ...
        # just for testing
        resp = df.choice[df.mouseName == mouse] == 'Right choice'  # Right choice is stim 1
        contrast = df.contrastRight[df.mouseName == mouse] - df.contrastLeft[df.mouseName == mouse]
        RT = df.choiceCompleteTime[df.mouseName == mouse] - df.goCueTime[df.mouseName == mouse]

        resp = df.choice[df.mouseName==self.mouse]=='Right choice' # Right choice is stim 1 / Left choice is 0
